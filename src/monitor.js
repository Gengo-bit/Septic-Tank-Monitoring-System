const firebaseConfig = {
  ...
  databaseURL:
    "https://septic-tank-capacity-default-rtdb.asia-southeast1.firebasedatabase.app",
...
auth.onAuthStateChanged((user) => {
  if (user) {
    ...
    const dataKey =
      tankNumber === "1" ? "septicTankData" : `septicTankData${tankNumber}`;

    // Check if this tank exists for this user
    database
      .ref(`users/${userId}/${dataKey}`)
      .once("value")
      .then((snapshot) => {
        if (snapshot.exists()) {
          ...
          document.querySelector(
            ".branding h1"
          ).textContent = `Septic Tank ${tankNumber}`;
        } else {
...
function fetchTankDimensions(userId) {
  database
    .ref(`users/${userId}/tankDimensions`)
    .once("value", (snapshot) => {
      if (snapshot.exists()) {
        const dimensions = snapshot.val();
        document.getElementById(
          "tankHeight"
        ).textContent = `${dimensions.tank_height} cm`;
        document.getElementById(
          "tankLength"
        ).textContent = `${dimensions.tank_length} cm`;
        document.getElementById(
          "tankWidth"
        ).textContent = `${dimensions.tank_width} cm`;
      } else {
...
function initializeApp(userId, dataKey) {
  ...
  const tankNumber =
    dataKey === "septicTankData" ? "1" : dataKey.replace("septicTankData", "");
  const distanceKey = tankNumber === "1" ? "distance" : `distance${tankNumber}`;

  database
    .ref(`users/${userId}/${distanceKey}`)
    .limitToLast(10)
    .on("value", (snapshot) => {
      if (snapshot.exists()) {
  ...
  database.ref(`users/${userId}/${dimensionsKey}`).once("value", (snapshot) => {
    if (snapshot.exists()) {
      ...
      document.getElementById(
        "tankHeight"
      ).textContent = `${dimensions.height}`;
      document.getElementById(
        "tankLength"
      ).textContent = `${dimensions.length}`;
      ...
      // Add event listeners
      document
        .getElementById("editDimensions")
        .addEventListener("click", () => {
          // Hide display paragraphs, show input fields
          document
            .querySelectorAll("#settingsForm p[id]")
            .forEach((p) => (p.style.display = "none"));
          document
            .querySelectorAll("#settingsForm input")
      ...
      document
        ...
        .addEventListener("click", () => {
          ...
          // Update database
          database
            .ref(`users/${userId}/${dimensionsKey}`)
...
function initializeCharts() {
  capacityChart = new Chart(
    document.getElementById("capacityChart").getContext("2d"),
    {
      type: "doughnut",
      data: {
      ...
      options: {
        ...
        plugins: {
          legend: {
            labels: {
              color: (context) =>
                context.chart.canvas.style.backgroundColor === "black"
                  ? "#D1D1D1"
                  : "#4A4A4A",
  ...
  historicalChart = new Chart(
    document.getElementById("historicalChart").getContext("2d"),
    {
      type: "line",
      data: {
        labels: [],
...
function handleSnapshot(snapshot) {
  ...
  const tankNumber =
    new URLSearchParams(window.location.search).get("tank") || "1";
  const distanceKey = tankNumber === "1" ? "distance" : `distance${tankNumber}`;

  // Fetch distance data separately since it's at the same level as septicTankData
  database
    .ref(`users/${userId}/${distanceKey}`)
    .once("value", (distanceSnapshot) => {
...
function updateCapacity(Cc) {
  ...
  chambers.forEach((water) => {
    ...
    water.style.background = `linear-gradient(to bottom, ${waterColor} 0%, ${adjustColor(
      waterColor,
      -20
    )} 100%)`;
  });
  ...
  document.getElementById(
    "capacity"
  ).innerHTML = `<span class="capacity-text">Capacity: ${Cc}%</span>`;
  ...
  if (Ti !== null && Ci !== null) {
    ...
    if (Q > 0) {
      if (Tf < 60) {
        document.getElementById(
          "prediction"
        ).innerHTML = `<span class="urgent-warning">The Septic Tank will be full in less than a minute!</span>`;
      } else {
        const Th = Tf;
      ...
      document.getElementById(
        "prediction"
      ).innerHTML = `<span class="negative-flow">Negative flow rate detected! The septic tank capacity is decreasing.</span>`;
    } else {
      document.getElementById(
        "prediction"
      ).innerHTML = `<span class="rate-too-low">No flow rate detected! The septic tank will not fill under current conditions.</span>`;
    }
  }
